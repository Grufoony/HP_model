...
Before defining the reward function it is appropriate to spend a few words to visualize how the RL algorithm described so far models the protein and its folds. Consider a complete episode and its associated the path $\varsigma = (\varsigma_{0}, \varsigma_1, ... , \varsigma_{k-1}) \ \ \varsigma_{i}$. To this path there will be an action sequence associated $a_{\varsigma} = (a_{\varsigma_{0}}, a_{\varsigma_{1}}, ..., a_{\varsigma_{k-2}})$. This sequence of actions is linked to the path since each element of both sequences appears in equation \ref{delta}. $a_{\varsigma}$ can be viewed as a possible bidimensional structure of the protein sequence. Therefore we can now associate to a certain action sequence, and by extension to a path $\varsigma$, the energy defined in section \ref{model}. 
